USE [CMRISE]
GO

CREATE OR ALTER  VIEW [dbo].[CC_HDR_V2] AS 
SELECT CH.[NUMERO]
      ,CH.[NOMBRE]
      ,CH.[TEMA]
	  ,TUV1.[SIGNIFICADO] TEMA_DESC
      ,CH.[HISTORIAL_CLINICO]
      ,CH.[DESCRIPCION_TECNICA]
      ,CH.[OPCION_INSEGURA]
      ,CH.[ETIQUETAS]
      ,CH.[NOTA]
      ,CH.[ESTATUS]
	  ,TUV2.SIGNIFICADO ESTATUS_DESC
      ,CH.[SOCIEDAD]
      ,CH.[FECHA_EFECTIVA_DESDE]
      ,CH.[FECHA_EFECTIVA_HASTA]
      ,CH.[CREADO_POR]
      ,CH.[FECHA_CREACION]
      ,CH.[ACTUALIZADO_POR]
      ,CH.[FECHA_ACTUALIZACION]
	  ,CPH.[TITULO] TITULO_PREGUNTA
	  ,CPH.TIPO_PREGUNTA
	  ,TUV3.SIGNIFICADO TIPO_PREGUNTA_DESC
	  ,ISNULL(CPH.MAX_PUNTUACION,0.00) MAX_PUNTUACION
	  ,CPH.TEMA_PREGUNTA
	  ,TUV4.SIGNIFICADO TEMA_PREGUNTA_DESC
  FROM [dbo].[CC_HDR] CH
      ,[dbo].[CC_PREGUNTAS_HDR] CPH
      ,[dbo].[TABLAS_UTILITARIAS_VALORES] TUV1
	  ,[dbo].[TABLAS_UTILITARIAS_VALORES] TUV2
	  ,[dbo].[TABLAS_UTILITARIAS_VALORES] TUV3
	  ,[dbo].[TABLAS_UTILITARIAS_VALORES] TUV4
 WHERE CH.NUMERO = CPH.NUMERO_CC_HDR
   AND TUV1.TIPO_TABLA='TEMA_DE_PREGUNTA'
   AND TUV1.CODIGO_TABLA = CH.TEMA
   AND TUV2.TIPO_TABLA='ESTATUS_MRQ'
   AND TUV2.CODIGO_TABLA = CH.ESTATUS
   AND TUV3.TIPO_TABLA = 'TIPO_PREGUNTA'
   AND TUV3.CODIGO_TABLA = CPH.TIPO_PREGUNTA
   AND TUV4.TIPO_TABLA='TEMA_DE_PREGUNTA'
   AND TUV4.CODIGO_TABLA = CPH.TEMA_PREGUNTA
GO


